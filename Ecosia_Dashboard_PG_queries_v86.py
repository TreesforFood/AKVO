...
b.submissionid_odk::text AS instance,
b.field_date,
'ODK' AS source_data,
'unregistered_farmers' AS data_form,
...
h.device_id,
h.submissionid_odk::text AS instance,
h.submission_date,
NULL AS submission_year,
'n/a' AS submissiontime,
...
p.device_id,
p.submissionid_odk::text AS instance,
p.submission_date,
NULL AS submission_year,
'n/a' AS submissiontime,
...
AND p.rn = 1
AND planting_date_added IS NOT NULL;
...
ADD COLUMN IF NOT EXISTS chloris_uploaded BOOLEAN DEFAULT 'false', -- Confirmation that the polygons are uploaded to Chloris
ADD COLUMN IF NOT EXISTS kanop_uploaded BOOLEAN DEFAULT 'false', -- Confirmation that the polygons are uploaded to Kanop
...
ADD COLUMN IF NOT EXISTS monitored_confirmation BOOLEAN DEFAULT False; -- Confirm if a site at least has been monitored once'''
...
WITH updates_polygon_odk AS (SELECT ecosia_site_id, test, remaped_polygon_planting_site, submission_date
FROM (
    SELECT
        ecosia_site_id,
		test,
...
WHERE NOT EXISTS (
    ...
    WHERE akvo_tree_registration_areas_edits.identifier_akvo = akvo_tree_registration_areas_updated.identifier_akvo
      AND akvo_tree_registration_areas_edits.instance = akvo_tree_registration_areas_updated.instance
)
...
SET
		...
		submission_year = t.submission_year,
		submissiontime = t.submissiontime,
		updated_at = t.updated_at,
		submitter = t.submitter,
		modifiedat = t.modifiedat,
		akvo_form_version = t.akvo_form_version,
		...
		id_planting_site = t.id_planting_site,
		land_title = t.land_title,
		name_village = t.name_village,
		name_region = t.name_region,
		name_owner = t.name_owner,
		photo_owner = t.photo_owner,
		gender_owner = t.gender_owner,
		objective_site = t.objective_site,
		site_preparation = t.site_preparation,
		planting_technique = t.planting_technique,
		planting_system = t.planting_system,
		remark = t.remark,
		nr_trees_option = t.nr_trees_option,
		planting_date = t.planting_date,
		tree_number = t.tree_number,
		estimated_area = t.estimated_area,
		calc_area = t.calc_area,
		lat_y = t.lat_y,
		lon_x = t.lon_x,
		...
		nr_trees_given_away = t.nr_trees_given_away,
		nr_trees_received = t.nr_trees_received,
		url_photo_receiver_trees = t.url_photo_receiver_trees,
		location_house_tree_receiver = t.location_house_tree_receiver,
		confirm_planting_location = t.confirm_planting_location,
		url_signature_tree_receiver = t.url_signature_tree_receiver,
		total_number_trees_received = t.total_number_trees_received,
		check_ownership_trees = t.check_ownership_trees,
		gender_tree_receiver = t.gender_tree_receiver,
		name_tree_receiver = t.name_tree_receiver,
		species_latin = t.species_latin,
		self_intersection = t.self_intersection,
		overlap = t.overlap,
		outside_country = t.outside_country,
		check_200_trees = t.check_200_trees,
		check_duplicate_polygons = t.check_duplicate_polygons,
		needle_shape = t.needle_shape,
		total_nr_geometric_errors = t.total_nr_geometric_errors,
		chloris_uploaded = t.chloris_uploaded,
		kanop_uploaded = t.kanop_uploaded,
		edit_confirmation = t.edit_confirmation,
...
ALTER TABLE qgis_tree_registration_points
ADD COLUMN IF NOT EXISTS fid SERIAL PRIMARY KEY; -- Needed in order to be able to edit in QGIS;'''
...
# # This is done to prevent that the centroids locations of polygons are mixing with the centroids
# # from sites that are < 200 trees. By doing to, the point layer in QGIS becomes clearer
# create_a1_remove_centroids_from_edits_layer_with_polygons = '''
# UPDATE akvo_tree_registration_areas_edits
# SET centroid_coord = NULL
# WHERE polygon IS NOT NULL;'''
#
# conn.commit()
...
# Update the UPDATE table with Chloris/Kanop uploads so that it is clear which are uploaded to Chloris and Kanop
update_updated_table_kanop_chloris = '''
...
calc_area = akvo_tree_registration_areas_edits.calc_area,
...
delete_confirmation = akvo_tree_registration_areas_edits.delete_confirmation

FROM akvo_tree_registration_areas_edits
WHERE akvo_tree_registration_areas_updated.identifier_akvo = akvo_tree_registration_areas_edits.identifier_akvo
AND akvo_tree_registration_areas_updated.instance = akvo_tree_registration_areas_edits.instance
AND akvo_tree_registration_areas_edits.edit_confirmation = TRUE

--OR akvo_tree_registration_areas_edits.delete_confirmation = TRUE) -- This condition is not needed. Why would you update if a delete is confirmed...?

AND (akvo_tree_registration_areas_edits.chloris_uploaded = FALSE -- This one is needed because one a polygon is uploaded to kanop OR chloris, a polygon modification in EDITS cannot enter the UPDATE table because the UPDATE table is the source for uploading data to kanop and chloris
OR akvo_tree_registration_areas_edits.kanop_uploaded = FALSE);'''

conn.commit()


# Clean the EDITS table. Remove instances that were not uploaded to Kanop/Chloris (and entered into the EDITS table) but later on WERE uploaded to Kanop/Chloris
# And also clean it from test data
# NOTE: this condition prevents also that you cannot change any kind of other data at a later stage, once uploaded to kanop or chloris, for example contract number, partner name, etc.
create_clean_edits_table = '''
DELETE FROM akvo_tree_registration_areas_edits
WHERE chloris_uploaded = True
...
THEN CAST(CONCAT(
	...
	SQRT(POWER(ASCII(LEFT(LOWER(t1.organisation),3)),4))) AS NUMERIC)
ELSE 0
...
THEN CAST(CONCAT(POWER(ASCII(RIGHT((LOWER(t1.organisation)),
			...
			LENGTH(t1.organisation) - POSITION('-' IN t1.organisation) - 2)),2),
		   	POWER(ASCII(RIGHT((LOWER(t1.organisation)),
			LENGTH(t1.organisation) - POSITION('-' IN t1.organisation) - 3)),2)) AS NUMERIC)
ELSE 0
...
ALTER TABLE superset_ecosia_kanop_chloris_results
ADD trend_kanop TEXT,
ADD trend_chloris TEXT,
ADD combined_trend TEXT;'''

conn.commit()


create_a1_remote_sensing_trend_results = '''
WITH stats AS (
  SELECT
    identifier_akvo,
	id_planting_site,
    ...
    -- For kanop
    SUM(kanop_above_ground_dry_biomass) AS sum_y_kanop,
    SUM((year_of_analisis - planting_year) * kanop_above_ground_dry_biomass) AS sum_xy_kanop,
    SUM((year_of_analisis - planting_year) * (year_of_analisis - planting_year)) AS sum_x2,

    -- For chloris
    SUM(chloris_above_ground_dry_biomass) AS sum_y_chloris,
    SUM((year_of_analisis - planting_year) * chloris_above_ground_dry_biomass) AS sum_xy_chloris
...
classify AS (SELECT
  ...
  -- Trend for kanop
  CASE
    WHEN (N * sum_xy_kanop - sum_x * sum_y_kanop) * 1.0 / NULLIF((N * sum_x2 - sum_x * sum_x), 0) > 0 THEN 'positive'
    WHEN (N * sum_xy_kanop - sum_x * sum_y_kanop) * 1.0 / NULLIF((N * sum_x2 - sum_x * sum_x), 0) < 0 THEN 'negative'
    ELSE 'neutral'
  END AS trend_kanop,

  -- Calculate slope for chloris
  (N * sum_xy_chloris - sum_x * sum_y_chloris) * 1.0 / NULLIF((N * sum_x2 - sum_x * sum_x), 0) AS slope_chloris,

  -- Trend for chloris
  CASE
    WHEN (N * sum_xy_chloris - sum_x * sum_y_chloris) * 1.0 / NULLIF((N * sum_x2 - sum_x * sum_x), 0) > 0 THEN 'positive'
    WHEN (N * sum_xy_chloris - sum_x * sum_y_chloris) * 1.0 / NULLIF((N * sum_x2 - sum_x * sum_x), 0) < 0 THEN 'negative'
    ELSE 'neutral'
  END AS trend_chloris
...
CASE
    WHEN trend_kanop = 'neutral' AND trend_chloris = 'neutral' THEN 'neutral'
    WHEN trend_kanop = 'positive' AND trend_chloris = 'positive' THEN 'positive'
    WHEN trend_kanop = 'negative' AND trend_chloris = 'negative' THEN 'negative'
    ELSE 'unknown'
END AS match_trend
...
SET
    trend_kanop = results.trend_kanop,
    trend_chloris = results.trend_chloris,
    combined_trend = results.match_trend
FROM results
WHERE results.identifier_akvo = superset_ecosia_kanop_chloris_results.identifier_akvo;'''
...
odk_tree_monitoring_main.submissionid_odk AS submissionid_odk_monitoring_main, -- This instance is now only to relate main monitorings with its monitoring repeats
...
plot_dates_monitorings_odk.submissionid_odk_monitoring_main,
...
plot_dates_monitorings_odk.monitoring_submission AS submission_date_monitoring,
...
odk_tree_monitoring_main.submissionid_odk AS submissionid_odk_monitoring_main,
odk_tree_monitoring_pcq.submissionid_odk AS submissionid_odk_monitoring_pcq
FROM odk_tree_monitoring_main
...
count_pcq_samples AS (SELECT
list_pcq_samples.ecosia_site_id,
table_label_strata.method_selection,
table_label_strata.label_strata,
COUNT(list_pcq_samples.submissionid_odk_monitoring_pcq) AS number_pcq_samples
FROM list_pcq_samples
LEFT JOIN table_label_strata
ON table_label_strata.submissionid_odk_monitoring_main = list_pcq_samples.submissionid_odk_monitoring_main
WHERE table_label_strata.method_selection = 'PCQ'
GROUP BY
table_label_strata.method_selection,
list_pcq_samples.ecosia_site_id,
table_label_strata.label_strata),
...
JOIN odk_tree_monitoring_pcq
ON odk_tree_monitoring_main.submissionid_odk = odk_tree_monitoring_pcq.submissionid_odk
WHERE odk_tree_monitoring_pcq.tree_distance_q1 IS NOT NULL
...
JOIN odk_tree_monitoring_pcq
ON odk_tree_monitoring_main.submissionid_odk = odk_tree_monitoring_pcq.submissionid_odk
WHERE odk_tree_monitoring_pcq.tree_distance_q2 IS NOT NULL
...
JOIN odk_tree_monitoring_pcq
ON odk_tree_monitoring_main.submissionid_odk = odk_tree_monitoring_pcq.submissionid_odk
WHERE odk_tree_monitoring_pcq.tree_distance_q3 IS NOT NULL
...
JOIN odk_tree_monitoring_pcq
ON odk_tree_monitoring_main.submissionid_odk = odk_tree_monitoring_pcq.submissionid_odk
WHERE odk_tree_monitoring_pcq.tree_distance_q4 IS NOT NULL),
...
JOIN odk_tree_monitoring_pcq
ON odk_tree_monitoring_main.submissionid_odk = odk_tree_monitoring_pcq.submissionid_odk
WHERE odk_tree_monitoring_pcq.tree_height_q1 IS NOT NULL
...
JOIN odk_tree_monitoring_pcq
ON odk_tree_monitoring_main.submissionid_odk = odk_tree_monitoring_pcq.submissionid_odk
WHERE odk_tree_monitoring_pcq.tree_height_q2 IS NOT NULL
...
JOIN odk_tree_monitoring_pcq
ON odk_tree_monitoring_main.submissionid_odk = odk_tree_monitoring_pcq.submissionid_odk
WHERE odk_tree_monitoring_pcq.tree_height_q3 IS NOT NULL
...
JOIN odk_tree_monitoring_pcq
ON odk_tree_monitoring_main.submissionid_odk = odk_tree_monitoring_pcq.submissionid_odk
WHERE odk_tree_monitoring_pcq.tree_height_q4 IS NOT NULL),
...
FROM merge_pcq_monitoring_dist
LEFT JOIN table_label_strata
ON merge_pcq_monitoring_dist.submissionid_odk = table_label_strata.submissionid_odk_monitoring_main
WHERE merge_pcq_monitoring_dist.pcq_results_merged_monitoring > 0
AND merge_pcq_monitoring_dist.pcq_results_merged_monitoring < 200
...
FROM merge_pcq_monitoring_hgt
LEFT JOIN table_label_strata
ON merge_pcq_monitoring_hgt.submissionid_odk = table_label_strata.submissionid_odk_monitoring_main
WHERE merge_pcq_monitoring_hgt.pcq_results_merged_monitoring_hgt > 0
AND merge_pcq_monitoring_hgt.pcq_results_merged_monitoring_hgt < 100
...
# Works well
create_a2_airtable = '''CREATE TABLE calc_tab_monitoring_calculations_per_site_merged_airtable AS

WITH plot_dates_monitorings_odk AS (SELECT
airtable_download_monitoring_results.identifier AS ecosia_site_id,
airtable_download_monitoring_results.identifier AS submissionid_monitoring_main, -- This instance is now only to relate main monitorings with its monitoring repeats
'This is real, valid data' AS test_monitoring,
--'unknown' AS id_planting_site,
akvo_tree_registration_areas_updated.id_planting_site AS id_planting_site,
airtable_download_monitoring_results.monitoring_date AS monitoring_submission,


-- Classify the options is needed to prevent seperate groups
'Tree count' AS method_selection,

'manual_input' AS procedure,

CASE
WHEN akvo_tree_registration_areas_updated.planting_date NOTNULL
THEN TO_DATE(akvo_tree_registration_areas_updated.planting_date, 'YYYY-MM-DD')
ELSE '9999-12-31'
END AS planting_date

FROM akvo_tree_registration_areas_updated
JOIN airtable_download_monitoring_results
ON airtable_download_monitoring_results.identifier = akvo_tree_registration_areas_updated.identifier_akvo),

table_label_strata AS (SELECT
plot_dates_monitorings_odk.ecosia_site_id,
plot_dates_monitorings_odk.submissionid_monitoring_main,
plot_dates_monitorings_odk.test_monitoring,
plot_dates_monitorings_odk.method_selection,
plot_dates_monitorings_odk.procedure,
plot_dates_monitorings_odk.planting_date,
...
calc_interm_results_tree_numbers_count_monitoring_airtable AS (SELECT
airtable_download_monitoring_results.identifier AS identifier_akvo,

CASE
WHEN akvo_tree_registration_areas_updated.contract_number NOTNULL AND akvo_tree_registration_areas_updated.id_planting_site NOTNULL AND akvo_tree_registration_areas_updated.name_owner NOTNULL
THEN CONCAT(akvo_tree_registration_areas_updated.contract_number,' - ', akvo_tree_registration_areas_updated.id_planting_site, ' - ', akvo_tree_registration_areas_updated.name_owner)
WHEN akvo_tree_registration_areas_updated.contract_number ISNULL OR akvo_tree_registration_areas_updated.id_planting_site ISNULL OR akvo_tree_registration_areas_updated.name_owner ISNULL
THEN CONCAT(akvo_tree_registration_areas_updated.contract_number,' - ', akvo_tree_registration_areas_updated.id_planting_site, ' - ', akvo_tree_registration_areas_updated.name_owner)
ELSE 'Display name cannot be generated because too many unknowns'
END AS displayname,

CASE
WHEN akvo_tree_registration_areas_updated.test = 'This is a test, this record can be deleted.'
THEN 'This is a test, this record can be deleted.'
WHEN akvo_tree_registration_areas_updated.test = 'valid_data'
THEN 'This is real, valid data'
WHEN akvo_tree_registration_areas_updated.test = 'This is real, valid data'
THEN 'This is real, valid data'
END AS test,
...
CASE
WHEN akvo_tree_registration_areas_updated.id_planting_site NOTNULL
THEN akvo_tree_registration_areas_updated.id_planting_site
ELSE 'unknown ID planting site'
END AS id_planting_site,

CASE
WHEN akvo_tree_registration_areas_updated.calc_area NOTNULL
THEN akvo_tree_registration_areas_updated.calc_area
ELSE 0
END AS calc_area,

CASE
WHEN akvo_tree_registration_areas_updated.tree_number NOTNULL
THEN akvo_tree_registration_areas_updated.tree_number
ELSE 0
END AS registered_tree_number,

'AIRTABLE' AS data_source,
'Monitoring' AS procedure,
'Tree count' AS data_collection_method,

CASE
WHEN airtable_download_monitoring_results.nr_trees_survived NOTNULL AND akvo_tree_registration_areas_updated.calc_area NOTNULL
THEN ROUND(100/NULLIF(SQRT(airtable_download_monitoring_results.nr_trees_survived/NULLIF(akvo_tree_registration_areas_updated.calc_area,0)),0),2)
WHEN airtable_download_monitoring_results.nr_trees_survived NOTNULL AND akvo_tree_registration_areas_updated.calc_area ISNULL
THEN ROUND(100/NULLIF(SQRT(airtable_download_monitoring_results.nr_trees_survived/NULLIF(akvo_tree_registration_areas_updated.calc_area,0)),0),2)
END AS avg_monitored_tree_distance,

CASE
WHEN airtable_download_monitoring_results.nr_trees_survived NOTNULL AND akvo_tree_registration_areas_updated.calc_area NOTNULL
THEN ROUND(AVG(airtable_download_monitoring_results.nr_trees_survived)/NULLIF(akvo_tree_registration_areas_updated.calc_area,0),0)
WHEN airtable_download_monitoring_results.nr_trees_survived NOTNULL AND akvo_tree_registration_areas_updated.calc_area ISNULL
THEN ROUND(AVG(airtable_download_monitoring_results.nr_trees_survived)/NULLIF(akvo_tree_registration_areas_updated.calc_area,0),0)
END AS avg_monitored_tree_density,

CASE
WHEN airtable_download_monitoring_results.nr_trees_survived NOTNULL
THEN ROUND(AVG(airtable_download_monitoring_results.nr_trees_survived),0)
ELSE 0
END AS nr_trees_monitored,
...
CASE
WHEN akvo_tree_registration_areas_updated.planting_date NOTNULL
THEN akvo_tree_registration_areas_updated.planting_date
ELSE 'unknown planting date'
END AS planting_date,

MAX(airtable_download_monitoring_results.monitoring_date) AS latest_monitoring_submission,
...
CASE
WHEN akvo_tree_registration_areas_updated.tree_number NOTNULL
THEN ROUND(AVG(airtable_download_monitoring_results.nr_trees_survived)/NULLIF(akvo_tree_registration_areas_updated.tree_number,0)*100,2)
ELSE 0
END AS perc_trees_survived,

0 AS avg_tree_height,

airtable_download_monitoring_results.notes

FROM airtable_download_monitoring_results
JOIN akvo_tree_registration_areas_updated
ON airtable_download_monitoring_results.identifier = akvo_tree_registration_areas_updated.identifier_akvo
JOIN table_label_strata
ON airtable_download_monitoring_results.identifier = table_label_strata.submissionid_monitoring_main
...
akvo_tree_registration_areas_updated.name_owner,
display_name,
airtable_download_monitoring_results.identifier,
airtable_download_monitoring_results.nr_trees_survived,
airtable_download_monitoring_results.notes)

-- Do a 'SELECT' to create the final table
SELECT * FROM calc_interm_results_tree_numbers_count_monitoring_airtable;'''
...
update_monitored_confirmation_update_table = '''
UPDATE akvo_tree_registration_areas_updated
SET monitored_confirmation = True
FROM calc_tab_monitoring_calculations_per_site_merged
WHERE calc_tab_monitoring_calculations_per_site_merged.identifier_akvo = akvo_tree_registration_areas_updated.identifier_akvo
AND calc_tab_monitoring_calculations_per_site_merged.label_strata > 0;'''

conn.commit()


update_monitored_confirmation_edits_table = '''
UPDATE akvo_tree_registration_areas_edits
SET monitored_confirmation = True
FROM calc_tab_monitoring_calculations_per_site_merged
WHERE calc_tab_monitoring_calculations_per_site_merged.identifier_akvo = akvo_tree_registration_areas_edits.identifier_akvo
AND calc_tab_monitoring_calculations_per_site_merged.label_strata > 0;'''
...
CTE_organisation_truncated AS (SELECT
    identifier_akvo,
	contract_number,
	organisation,
    split_part(organisation, ' - ', 1) AS main_organisation
FROM
    akvo_tree_registration_areas_updated
),

CTE_country_random_select AS (
WITH RankedContracts AS (
    SELECT
        ...
        ROUND(
            CASE
                WHEN contract_number - FLOOR(contract_number) > 0 THEN FLOOR(contract_number)
        ...
        ROW_NUMBER() OVER (
            ...
            ORDER BY RANDOM()  -- Use RAND() or NEWID() depending on your SQL dialect
        ) AS rn
    FROM akvo_tree_registration_areas_updated
)
SELECT
...
SELECT
lower(CTE_country_random_select.country) as name_country,
lower(CTE_organisation_truncated.main_organisation) as organisation,

    ROUND(
        CASE
            WHEN akvo_tree_registration_areas_updated.contract_number - FLOOR(akvo_tree_registration_areas_updated.contract_number) > 0 THEN FLOOR(akvo_tree_registration_areas_updated.contract_number)
            ELSE akvo_tree_registration_areas_updated.contract_number
        END,
        2
    ) AS contract_number,
...
FROM akvo_tree_registration_areas_updated
JOIN CTE_organisation_truncated
ON akvo_tree_registration_areas_updated.identifier_akvo = CTE_organisation_truncated.identifier_akvo
JOIN CTE_country_random_select
ON akvo_tree_registration_areas_updated.contract_number = CTE_country_random_select.rounded_contract
WHERE (akvo_tree_registration_areas_updated.test = '' OR akvo_tree_registration_areas_updated.test = 'This is real, valid data')
AND NOT akvo_tree_registration_areas_updated.id_planting_site = 'placeholder' AND NOT CTE_country_random_select.country = '' AND NOT CTE_organisation_truncated.organisation = ''
GROUP BY
akvo_tree_registration_areas_updated.contract_number,
CTE_country_random_select.country,
CTE_organisation_truncated.main_organisation),
...
CASE
WHEN a.native_exotic = 'N'
THEN 'native'
WHEN a.native_exotic = 'E'
THEN 'exotic'
ELSE 'unknown'
END AS native_exotic_full,
...
WHERE d.test != 'This is a test, this record can be deleted.'),

total_nr_trees AS (
    SELECT
        contract_number,
        SUM(CASE WHEN native_exotic_full IN ('native', 'exotic') THEN nr_trees_per_species ELSE 0 END) AS total_nr_trees
...
CTE_results_percentages_exotic_native AS (
    SELECT
        c.contract_number,
        ROUND(SUM(CASE WHEN native_exotic_full = 'native' THEN nr_trees_per_species ELSE 0 END) * 100.0 / NULLIF(t.total_nr_trees, 0), 1) AS percentage_native,
        ROUND(SUM(CASE WHEN native_exotic_full = 'exotic' THEN nr_trees_per_species ELSE 0 END) * 100.0 / NULLIF(t.total_nr_trees, 0), 1) AS percentage_exotic
    FROM CTE_percentage_exotic_native c
    JOIN total_nr_trees t ON c.contract_number = t.contract_number
...
--xxx
-- Eventueel onderstaande vervangen (code voor partners) door de onderstaande 'hash' functie:
--SELECT
--   DISTINCT organisation,
--
--        SUBSTRING(MD5(LOWER(t.organisation)) FROM 1 FOR 15)
--     AS partnercode_main
--FROM superset_ecosia_tree_registration t
--ORDER BY organisation;
-- Bovenstaande hash is robuster en genereerd unieke codes. Ook voor subpartners doen. Dit overal doen!!
...
odk_tree_number_per_species AS (SELECT
submissionid_odk, species_name_latin, nr_trees_per_species
FROM odk_tree_registration_tree_species
),
...
odk_tree_number_per_species.nr_trees_per_species AS number_species
...
JOIN odk_tree_number_per_species
ON odk_tree_number_per_species.submissionid_odk = odk_tree_registration_main.submissionid_odk
AND odk_tree_number_per_species.species_name_latin = seperate_tree_species_registration_odk.species_registered_odk)
...
-- Here we convert the photo (GPS) locations (AKVO TREE REGISTRATION) from WKT format to geojson string format that can be read by superset
wkt_photo_registration_gps_to_geojson_akvo AS
(SELECT
...
END AS test,
    ...
    CASE
        ...
        ELSE CAST(
            CONCAT(
                POWER(ASCII(LEFT(LOWER(odk_tree_registration_main.organisation),1)),3),
                POWER(ASCII(LEFT(LOWER(odk_tree_registration_main.organisation),2)),2),
                SQRT(POWER(ASCII(LEFT(LOWER(odk_tree_registration_main.organisation),3)),4))
            ) AS NUMERIC
        )
    END AS partnercode_main,
    ...
    CASE
        ...
        ELSE CAST(
            CONCAT(
                POWER(ASCII(RIGHT((LOWER(odk_tree_registration_main.organisation)), LENGTH(odk_tree_registration_main.organisation) - POSITION('-' IN odk_tree_registration_main.organisation) - 1)),3),
                POWER(ASCII(RIGHT((LOWER(odk_tree_registration_main.organisation)), LENGTH(odk_tree_registration_main.organisation) - POSITION('-' IN odk_tree_registration_main.organisation) - 2)),2),
                POWER(ASCII(RIGHT((LOWER(odk_tree_registration_main.organisation)), LENGTH(odk_tree_registration_main.organisation) - POSITION('-' IN odk_tree_registration_main.organisation) - 3)),2)
            ) AS NUMERIC
        )
    END AS partnercode_sub,
    LOWER(odk_tree_registration_main.organisation) AS organisation,
    CASE
        WHEN POSITION('-' IN odk_tree_registration_main.organisation) > 0 THEN LOWER(left(odk_tree_registration_main.organisation, strpos(odk_tree_registration_main.organisation, '-') - 2))
        WHEN POSITION(' -' IN odk_tree_registration_main.organisation) > 0 THEN LOWER(left(odk_tree_registration_main.organisation, strpos(odk_tree_registration_main.organisation, '-') - 1))
        WHEN POSITION('/' IN odk_tree_registration_main.organisation) > 0 THEN LOWER(left(odk_tree_registration_main.organisation, strpos(odk_tree_registration_main.organisation, '/') - 1))
        WHEN POSITION(' /' IN odk_tree_registration_main.organisation) > 0 THEN LOWER(left(odk_tree_registration_main.organisation, strpos(odk_tree_registration_main.organisation, '/') - 1))
        ELSE LOWER(odk_tree_registration_main.organisation)
    END AS partner,
    CASE
        ...
        WHEN POSITION('-' IN odk_tree_registration_main.organisation) > 0 THEN LOWER(right(odk_tree_registration_main.organisation, (LENGTH(odk_tree_registration_main.organisation) - strpos(odk_tree_registration_main.organisation, '-'))))
        WHEN POSITION('/' IN odk_tree_registration_main.organisation) > 0 THEN LOWER(right(odk_tree_registration_main.organisation, (LENGTH(odk_tree_registration_main.organisation) - strpos(odk_tree_registration_main.organisation, '/'))))
        WHEN POSITION('/ ' IN odk_tree_registration_main.organisation) > 0 THEN LOWER(right(odk_tree_registration_main.organisation, (LENGTH(odk_tree_registration_main.organisation) - strpos(odk_tree_registration_main.organisation, '/')-1)))
        ELSE ''
    END AS sub_partner,
    ...
    jsonb_build_object(
        ...
        'features', json_agg(
            json_build_object(
                'type', 'Feature',
                'properties', 'Photo registration',
                'geometry', ST_AsGeoJSON(odk_tree_registration_photos.photo_gps_location)::json
            )
        )
    )::text AS superset_geojson
FROM odk_tree_registration_photos
JOIN odk_tree_registration_main ON odk_tree_registration_photos.submissionid_odk = odk_tree_registration_main.submissionid_odk
WHERE odk_tree_registration_photos.photo_gps_location IS NOT NULL
  AND odk_tree_registration_main.organisation IS NOT NULL
  AND odk_tree_registration_main.organisation != ''
GROUP BY
    odk_tree_registration_photos.submissionid_odk,
    odk_tree_registration_photos.repeatid_odk,
    odk_tree_registration_main.organisation,
    odk_tree_registration_main.test,
    odk_tree_registration_main.contract_number,
    odk_tree_registration_main.id_planting_site,
    odk_tree_registration_main.submission_date,
    odk_tree_registration_main.country),
...
-- Here we convert the photo (GPS) locations of added trees (ODK ADDED TREES) from WKT format to geojson string format that can be read by superset. Added trees is labelled as 'photo registration' since adding photos to a site is basically part of the site registration process.
wkt_added_photo_registration_gps_to_geojson_odk AS
(SELECT
    odk_tree_monitoring_add_photos.submissionid_odk,
    odk_tree_monitoring_add_photos.repeatid_odk::TEXT,
    odk_tree_monitoring_main.submission_date,
    CASE
        WHEN odk_tree_monitoring_main.test = 'test_data' THEN 'yes'
        WHEN odk_tree_monitoring_main.test = 'This is a test, this record can be deleted.' THEN 'yes'
        WHEN odk_tree_monitoring_main.test = 'This is a test, this record can be deleted' THEN 'yes'
        WHEN odk_tree_monitoring_main.test = 'xxxxx' THEN 'yes'
        WHEN odk_tree_monitoring_main.test = '' THEN 'no'
        WHEN odk_tree_monitoring_main.test = 'This is real, valid data\r' THEN 'no'
        WHEN odk_tree_monitoring_main.test = 'valid_data' THEN 'no'
        WHEN odk_tree_monitoring_main.test = 'Valid data' THEN 'no'
        WHEN odk_tree_monitoring_main.test = 'This is real, valid data' THEN 'no'
        WHEN odk_tree_monitoring_main.test IS NULL THEN 'no'
    END AS test,
    LOWER(odk_tree_monitoring_main.country) AS country,
    -- Safe calculation for partnercode_main (handles empty organisation)
    CASE
        WHEN odk_tree_monitoring_main.organisation = '' OR odk_tree_monitoring_main.organisation IS NULL THEN 0
        ELSE CAST(
            CONCAT(
                POWER(ASCII(LEFT(LOWER(odk_tree_monitoring_main.organisation),1)),3),
                POWER(ASCII(LEFT(LOWER(odk_tree_monitoring_main.organisation),2)),2),
                SQRT(POWER(ASCII(LEFT(LOWER(odk_tree_monitoring_main.organisation),3)),4))
            ) AS NUMERIC
        )
    END AS partnercode_main,
    -- Safe calculation for partnercode_sub (handles empty or non-hyphen organisation)
    CASE
        WHEN odk_tree_monitoring_main.organisation = '' OR odk_tree_monitoring_main.organisation IS NULL THEN 0
        WHEN POSITION('-' IN odk_tree_monitoring_main.organisation) = 0 THEN 0
        ELSE CAST(
            CONCAT(
                POWER(ASCII(RIGHT((LOWER(odk_tree_monitoring_main.organisation)), LENGTH(odk_tree_monitoring_main.organisation) - POSITION('-' IN odk_tree_monitoring_main.organisation) - 1)),3),
                POWER(ASCII(RIGHT((LOWER(odk_tree_monitoring_main.organisation)), LENGTH(odk_tree_monitoring_main.organisation) - POSITION('-' IN odk_tree_monitoring_main.organisation) - 2)),2),
                POWER(ASCII(RIGHT((LOWER(odk_tree_monitoring_main.organisation)), LENGTH(odk_tree_monitoring_main.organisation) - POSITION('-' IN odk_tree_monitoring_main.organisation) - 3)),2)
            ) AS NUMERIC
        )
    END AS partnercode_sub,
    LOWER(odk_tree_monitoring_main.organisation) AS organisation,
    CASE
        WHEN POSITION('-' IN odk_tree_monitoring_main.organisation) > 0 THEN LOWER(left(odk_tree_monitoring_main.organisation, strpos(odk_tree_monitoring_main.organisation, '-') - 2))
        WHEN POSITION(' -' IN odk_tree_monitoring_main.organisation) > 0 THEN LOWER(left(odk_tree_monitoring_main.organisation, strpos(odk_tree_monitoring_main.organisation, '-') - 1))
        WHEN POSITION('/' IN odk_tree_monitoring_main.organisation) > 0 THEN LOWER(left(odk_tree_monitoring_main.organisation, strpos(odk_tree_monitoring_main.organisation, '/') - 1))
        WHEN POSITION(' /' IN odk_tree_monitoring_main.organisation) > 0 THEN LOWER(left(odk_tree_monitoring_main.organisation, strpos(odk_tree_monitoring_main.organisation, '/') - 1))
        ELSE LOWER(odk_tree_monitoring_main.organisation)
    END AS partner,
    CASE
        WHEN POSITION('- ' IN odk_tree_monitoring_main.organisation) > 0 THEN LOWER(right(odk_tree_monitoring_main.organisation, (LENGTH(odk_tree_monitoring_main.organisation) - strpos(odk_tree_monitoring_main.organisation, '-')-1)))
        WHEN POSITION('-' IN odk_tree_monitoring_main.organisation) > 0 THEN LOWER(right(odk_tree_monitoring_main.organisation, (LENGTH(odk_tree_monitoring_main.organisation) - strpos(odk_tree_monitoring_main.organisation, '-'))))
        WHEN POSITION('/' IN odk_tree_monitoring_main.organisation) > 0 THEN LOWER(right(odk_tree_monitoring_main.organisation, (LENGTH(odk_tree_monitoring_main.organisation) - strpos(odk_tree_monitoring_main.organisation, '/'))))
        WHEN POSITION('/ ' IN odk_tree_monitoring_main.organisation) > 0 THEN LOWER(right(odk_tree_monitoring_main.organisation, (LENGTH(odk_tree_monitoring_main.organisation) - strpos(odk_tree_monitoring_main.organisation, '/')-1)))
        ELSE ''
    END AS sub_partner,
    odk_tree_monitoring_main.contract_number_added_trees AS sub_contract,
    odk_tree_monitoring_main.id_planting_site,
    'no display name' AS display_name,
    'Photo registration' AS procedure,
    'Photo registration' AS geolocation_type,
    jsonb_build_object(
        ...
        'features', json_agg(
            json_build_object(
                'type', 'Feature',
                'properties', 'Photo registration',
                'geometry', ST_AsGeoJSON(odk_tree_monitoring_add_photos.add_photo_gps_location)::json
            )
        )
    )::text AS superset_geojson
FROM odk_tree_monitoring_add_photos
JOIN odk_tree_monitoring_main
ON odk_tree_monitoring_add_photos.submissionid_odk = odk_tree_monitoring_main.submissionid_odk
WHERE odk_tree_monitoring_add_photos.add_photo_gps_location IS NOT NULL
  AND odk_tree_monitoring_main.organisation IS NOT NULL
  AND odk_tree_monitoring_main.organisation != ''
GROUP BY
    odk_tree_monitoring_add_photos.submissionid_odk,
    odk_tree_monitoring_add_photos.repeatid_odk,
    odk_tree_monitoring_main.organisation,
    odk_tree_monitoring_main.test,
    odk_tree_monitoring_main.contract_number_added_trees,
    odk_tree_monitoring_main.id_planting_site,
    odk_tree_monitoring_main.submission_date,
    odk_tree_monitoring_main.country),


-- Here we convert the photo (GPS) locations of counted trees (ODK COUNT TREES) from WKT format to geojson string format that can be read by superset. Added trees is labelled as 'photo registration' since adding photos to a site is basically part of the site registration process.
wkt_count_photo_registration_gps_to_geojson_odk AS (

SELECT
    odk_tree_monitoring_count_photos.submissionid_odk,
    odk_tree_monitoring_count_photos.repeatid_odk::TEXT,
    odk_tree_monitoring_main.submission_date,
    CASE
        WHEN odk_tree_monitoring_main.test = 'test_data' THEN 'yes'
        WHEN odk_tree_monitoring_main.test = 'This is a test, this record can be deleted.' THEN 'yes'
    ...
    odk_tree_monitoring_main.id_planting_site,
    'no display name' AS display_name,
    'Photo count monitoring' AS procedure,
    'Photo count monitoring' AS geolocation_type,
    jsonb_build_object(
        'type', 'FeatureCollection',
        'features', json_agg(
            json_build_object(
                'type', 'Feature',
                'properties', 'Photo count monitoring',
                'geometry', ST_AsGeoJSON(odk_tree_monitoring_count_photos.tree_photo_gps)::json
            )
        )
    )::text AS superset_geojson
FROM odk_tree_monitoring_count_photos
JOIN odk_tree_monitoring_main
ON odk_tree_monitoring_count_photos.submissionid_odk = odk_tree_monitoring_main.submissionid_odk
WHERE odk_tree_monitoring_count_photos.tree_photo_gps IS NOT NULL
  AND odk_tree_monitoring_main.organisation IS NOT NULL
  AND odk_tree_monitoring_main.organisation != ''
GROUP BY
    odk_tree_monitoring_count_photos.submissionid_odk,
    odk_tree_monitoring_count_photos.repeatid_odk,
    odk_tree_monitoring_main.organisation,
    odk_tree_monitoring_main.test,
    odk_tree_monitoring_main.contract_number_added_trees,
    odk_tree_monitoring_main.id_planting_site,
...
-- Here we convert the photo (GPS) locations of PCQ trees (ODK PCQ TREES) from WKT format to geojson string format that can be read by superset. Added trees is labelled as 'photo registration' since adding photos to a site is basically part of the site registration process.
wkt_pcq_samples_odk_monitoring_to_geojson AS (
SELECT
    odk_tree_monitoring_pcq.submissionid_odk,
    odk_tree_monitoring_pcq.repeatid_odk::TEXT,
    odk_tree_monitoring_main.submission_date,
    CASE
        WHEN odk_tree_monitoring_main.test = 'test_data' THEN 'yes'
        WHEN odk_tree_monitoring_main.test = 'This is a test, this record can be deleted.' THEN 'yes'
...
FROM odk_tree_monitoring_pcq
JOIN odk_tree_monitoring_main
ON odk_tree_monitoring_pcq.submissionid_odk = odk_tree_monitoring_main.submissionid_odk
WHERE odk_tree_monitoring_pcq.gps_pcq_sample IS NOT NULL
  AND odk_tree_monitoring_main.organisation IS NOT NULL
  AND odk_tree_monitoring_main.organisation != ''
GROUP BY
    odk_tree_monitoring_pcq.submissionid_odk,
    odk_tree_monitoring_pcq.repeatid_odk,
    odk_tree_monitoring_main.organisation,
    odk_tree_monitoring_main.test,
    odk_tree_monitoring_main.contract_number_added_trees,
    odk_tree_monitoring_main.id_planting_site,
...
UNION ALL
SELECT * FROM wkt_photo_registration_gps_to_geojson_akvo
UNION ALL
SELECT * FROM wkt_photo_registration_gps_to_geojson_odk
UNION ALL
SELECT * FROM wkt_added_photo_registration_gps_to_geojson_odk
UNION ALL
SELECT * FROM wkt_count_photo_registration_gps_to_geojson_odk
UNION ALL
SELECT * FROM wkt_pcq_samples_odk_monitoring_to_geojson
UNION ALL
...
AS WITH merge_odk_tree_monitoring_pcq AS

(SELECT submissionid_odk, tree_photo_q1 AS photo_url, gps_pcq_sample AS photo_gps_location FROM ODK_Tree_monitoring_pcq
UNION ALL
SELECT submissionid_odk, tree_photo_q2 AS photo_url, gps_pcq_sample AS photo_gps_location FROM ODK_Tree_monitoring_pcq
UNION ALL
SELECT submissionid_odk, tree_photo_q3 AS photo_url, gps_pcq_sample AS photo_gps_location FROM ODK_Tree_monitoring_pcq
UNION ALL
SELECT submissionid_odk, tree_photo_q4 AS photo_url, gps_pcq_sample AS photo_gps_location FROM ODK_Tree_monitoring_pcq),

merge_odk_tree_monitoring_add_photos AS (
SELECT submissionid_odk, add_photo_1 AS photo_url, add_photo_gps_location AS photo_gps_location FROM odk_tree_monitoring_add_photos
UNION ALL
SELECT submissionid_odk, add_photo_2 AS photo_url, add_photo_gps_location AS photo_gps_location FROM odk_tree_monitoring_add_photos
UNION ALL
SELECT submissionid_odk, add_photo_3 AS photo_url, add_photo_gps_location AS photo_gps_location FROM odk_tree_monitoring_add_photos
UNION ALL
SELECT submissionid_odk, add_photo_4 AS photo_url, add_photo_gps_location AS photo_gps_location FROM odk_tree_monitoring_add_photos)
...
akvo_tree_monitoring_photos.instance::TEXT,
...
AKVO_Tree_external_audits_photos.instance::TEXT,
...
a.submitter,
a.submission_date AS submission,
akvo_tree_registration_areas_updated.id_planting_site,
b.photo_url AS photo_url,
b.photo_gps_location AS photo_location

FROM akvo_tree_registration_areas_updated
JOIN odk_tree_monitoring_main a
ON akvo_tree_registration_areas_updated.identifier_akvo = a.ecosia_site_id
JOIN merge_odk_tree_monitoring_pcq b
ON a.submissionid_odk = b.submissionid_odk
...
JOIN odk_tree_monitoring_main a
ON akvo_tree_registration_areas_updated.identifier_akvo = a.ecosia_site_id
JOIN merge_odk_tree_monitoring_add_photos b
ON a.submissionid_odk = b.submissionid_odk;
...
--ingevoegd:
DROP POLICY IF EXISTS ecosia_edit_policy ON qgis_tree_registration_areas;
DROP POLICY IF EXISTS ecosia_edit_policy ON qgis_tree_registration_points;
---
...
GRANT SELECT ON TABLE public.qgis_tree_registration_points TO ecosia_editing;
---
...
cur.execute(create_a2_airtable)
cur.execute(create_a2_merge_akvo_odk)
cur.execute(update_monitored_confirmation_update_table)
cur.execute(update_monitored_confirmation_edits_table)
...
cur.execute(create_49a_remote_sensing_trend_results_contract_level)
