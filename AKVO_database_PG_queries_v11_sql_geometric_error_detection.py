...
UPDATE akvo_tree_registration_areas_updated SET check_duplicate_polygons = NULL;

-- Create temporary tables with valid geometries
CREATE TEMPORARY TABLE akvo_tree_registration_areas_updated_temp_table3 AS
(
    SELECT
        identifier_akvo,
        id_planting_site,
        ...
        ST_MakeValid(polygon::geometry) AS pol,
        ST_Transform(polygon::geometry, 4326) AS polgeo
    FROM akvo_tree_registration_areas_updated
    WHERE polygon IS NOT NULL
);

CREATE TEMPORARY TABLE akvo_tree_registration_areas_updated_temp_table4 AS
(
    SELECT
        identifier_akvo,
        id_planting_site,
        ...
        country,
        total_nr_geometric_errors,
        ST_Transform(polgeo::geometry, 4326) AS polgeo
    FROM akvo_tree_registration_areas_updated_temp_table3
    WHERE polgeo IS NOT NULL AND ST_IsValid(polgeo::geometry)
);
...
-- Identify all duplicate pairs (both directions)
WITH duplicate_polygons AS (
    SELECT
        a.identifier_akvo AS identifier_akvo_a,
        b.identifier_akvo AS identifier_akvo_b
    FROM
        akvo_tree_registration_areas_updated_temp_table4 a
    JOIN
        akvo_tree_registration_areas_updated_temp_table4 b
    ON
        ST_EQUALS(a.polgeo, b.polgeo)
        AND a.identifier_akvo < b.identifier_akvo  -- Avoid self-pairs and duplicate rows
),

-- Aggregate all duplicates for each polygon (both directions)
transpose_duplicate_identifiers AS (
    SELECT
        identifier_akvo_a AS identifier_akvo,
        STRING_AGG(identifier_akvo_b::text, ' | ') AS duplicates
    FROM
        duplicate_polygons
    GROUP BY
        identifier_akvo_a
    ...
    SELECT
        identifier_akvo_b AS identifier_akvo,
        STRING_AGG(identifier_akvo_a::text, ' | ') AS duplicates
    FROM
        duplicate_polygons
    GROUP BY
        identifier_akvo_b
...
-- Update the main table with all duplicates
UPDATE akvo_tree_registration_areas_updated
SET check_duplicate_polygons = (
    SELECT STRING_AGG(duplicates, ' | ')
    FROM transpose_duplicate_identifiers
    WHERE transpose_duplicate_identifiers.identifier_akvo = akvo_tree_registration_areas_updated.identifier_akvo
)
WHERE identifier_akvo IN (SELECT identifier_akvo FROM transpose_duplicate_identifiers);
