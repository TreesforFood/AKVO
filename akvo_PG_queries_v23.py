...
CREATE TABLE CALC_TAB_monitoring_calculations_per_site AS

-- Create CTE with aggregation on PCQ instances on MONITORING. This is important to get 1 single label strata per instance
...
-- Create CTE with COUNT instances on MONITORING. These should be the count repeat instances in GROUP 2, which are the counts per species (table 'akvo_tree_monitoring_counts').
...
-- Create CTE with aggregation on PCQ instances on AUDITS. This is important to get 1 single label strata per instance
pcq_instances_grouped_audit AS (SELECT
...
WHERE (AKVO_Tree_external_audits_areas.test = '' OR AKVO_Tree_external_audits_areas.test = 'This is real, valid data')),

-- Create CTE with COUNT instances on AUDITS. These should be the count repeat instances in GROUP 2, which are the counts per species (table 'akvo_tree_monitoring_counts').
-- IF these values don't exists, the query should use the tree number in the main table (akvo_tree_audit_areas)
count_instances_grouped_audit AS (SELECT
AKVO_Tree_external_audits_areas.identifier_akvo,
AKVO_Tree_external_audits_areas.instance,
akvo_tree_registration_areas.id_planting_site,
AKVO_Tree_external_audits_areas.submission
...
WHERE (akvo_tree_registration_areas.test = ''
OR akvo_tree_registration_areas.test = 'This is real, valid data')),


--UNION OF COUNTS and PCQ's of MONITORINGS and AUDITS and REGISTRATION into 1 table. Instances are grouped. This table enters the calculate to add strata (= days distance between instances) to instances
monitoring_instances_grouped AS
(SELECT * FROM pcq_instances_grouped
UNION
SELECT * FROM count_instances_grouped
UNION
SELECT * FROM pcq_instances_grouped_audit
UNION
SELECT * FROM count_instances_grouped_audit
UNION
...
calc_interm_results_tree_numbers_pcq_audit.monitoring_strata,
'Audit' AS procedure,
'PCQ' AS method,
SUM(calc_interm_results_tree_numbers_pcq_audit.nr_samples_pcq_audit) AS nr_samples_pcq_audit,
SUM(calc_interm_results_tree_numbers_pcq_audit.nr_trees_pcq_audit) AS monitored_tree_number,
...
calc_interm_results_tree_numbers_audits.monitoring_strata,
'Audit' AS procedure,
'tree count' as method,
0 AS nr_samples_pcq,
ROUND(AVG(calc_interm_results_tree_numbers_audits.audited_tree_number),0) as monitored_tree_number,
0 AS avg_tree_distance,
0 AS avg_tree_density,
...
monitoring_tree_numbers.monitoring_strata AS label_strata,
monitoring_tree_numbers.procedure,
monitoring_tree_numbers.method AS method_data_collection,
monitoring_tree_numbers.submission AS latest_monitoring_submission,
akvo_tree_registration_areas.id_planting_site,
...
monitoring_tree_numbers.monitored_tree_number AS nr_trees_monitored,
monitoring_tree_numbers.avg_perc_survived_trees AS perc_trees_survived
...
akvo_tree_registration_areas.submission AS latest_monitoring_submission,
akvo_tree_registration_areas.id_planting_site,
akvo_tree_registration_areas.contract_number,
akvo_tree_registration_areas.organisation,
akvo_tree_registration_areas.tree_number AS "nr trees initially registered",
...
akvo_tree_registration_areas.tree_number AS nr_trees_monitored,
100 AS perc_trees_survived
