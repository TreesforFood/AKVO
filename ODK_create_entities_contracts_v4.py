...
count = 0
list_contracts_open_for_reporting = []
tuple_contracts_open_for_reporting = () # create a tuple from the list to prevent duplicates
    ...
    for y in x:
        ...
        if open_for_reporting is True:
            try:
                ...
                maincontractnr_dec = str(maincontractnr + '.00')
            except (KeyError, IndexError):
                continue
            list_contracts_open_for_reporting.append(maincontractnr)
            tuple_contracts_open_for_reporting = tuple(list_contracts_open_for_reporting)
            ...
            VALUES (%s,%s,%s,%s)''', (label, organisation, maincontractnr_dec, country))
            conn.commit()
...
count = 0
print(tuple_contracts_open_for_reporting)

# Get the results from the pagination function
for x in result:
    for y in x:
        contract_number_odk = str(y['fields']['Contractnr_odk'][0])
        if contract_number_odk in tuple_contracts_open_for_reporting:
            maincontract = str(contract_number_odk)
