...
cur.execute('''DROP TABLE IF EXISTS getodk_entities_upload_AKVO_table_nurseries;''')
cur.execute('''DROP TABLE IF EXISTS getodk_entities_upload_ODK_table_nurseries;''')
cur.execute('''DROP TABLE IF EXISTS getodk_entities_upload_MERGE_ALL_table_nurseries;''')
...
cur.execute(
'''CREATE TABLE getodk_entities_upload_AKVO_table_nurseries AS
...
cur.execute('''SELECT geometry, ecosia_nursery_id FROM getodk_entities_upload_AKVO_table_nurseries;''')
...
for key,value in dict.items():
    cur.execute('''UPDATE getodk_entities_upload_AKVO_table_nurseries
    SET geometry = %s
...
# Also duplicate labels should be avoided!
cur.execute(
'''CREATE TABLE getodk_entities_upload_ODK_table_nurseries AS
(SELECT DISTINCT(CONCAT('Organisation: ', LOWER(organisation), ' | Nursery name: ', nursery_registration_name, ' | Nursery id:', submissionid_odk)) AS label,

submissionid_odk AS ecosia_nursery_id,

CASE -- Fields can not be empty when uploaded to the entity list of ODK. If so, ODK gives a 'no string' error
WHEN contract_number_tree_production NOTNULL
THEN contract_number_tree_production::TEXT
ELSE ''
END AS contract_number,

CASE -- Fields can not be empty when uploaded to the entity list of ODK. If so, ODK gives a 'no string' error
WHEN nursery_registration_name NOTNULL
THEN nursery_registration_name
ELSE 'nursery_name_unknown'
END AS name_nursery,
...
CASE -- Fields can not be empty when uploaded to the entity list of ODK. If so, ODK gives a 'no string' error
WHEN submission_date NOTNULL
THEN submission_date::TEXT
ELSE 'registration_date_unknown'
END AS nursery_registration_date,

CASE -- Fields can not be empty when uploaded to the entity list of ODK. If so, ODK gives a 'no string' error
WHEN user_name NOTNULL
THEN user_name
ELSE 'name_submitter_unknown'
END AS user_name_enumerator,

CONCAT(lat_y, ' ', lon_x)::TEXT AS geometry

FROM odk_nursery_registration_main
WHERE test_data_yes_no = 'valid_data'
AND lat_y NOTNULL
AND lon_x NOTNULL)''')

conn.commit()

cur.execute('''CREATE TABLE getodk_entities_upload_MERGE_ALL_table_nurseries
AS SELECT * FROM getodk_entities_upload_AKVO_table_nurseries
UNION ALL
SELECT * FROM getodk_entities_upload_ODK_table_nurseries''')

conn.commit()

# Select all rows and fetch them all
cur.execute('''SELECT * FROM getodk_entities_upload_MERGE_ALL_table_nurseries''')
conn.commit()
...
client.entities.merge(entities_list, entity_list_name='monitoring_nurseries', project_id=1, match_keys=None, add_new_properties=True, update_matched=True, delete_not_matched=True, source_label_key='label', source_keys=None,create_source=None, source_size=None)
