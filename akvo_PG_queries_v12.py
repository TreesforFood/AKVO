...
-- Create CTE with COUNT instances. There is always 1 COUNT instance (no repeat)
...
akvo_tree_monitoring_areas.submission
FROM AKVO_Tree_monitoring_areas
JOIN akvo_tree_monitoring_counts ON AKVO_Tree_monitoring_areas.instance = akvo_tree_monitoring_counts.instance
JOIN AKVO_Tree_registration_areas ON AKVO_Tree_registration_areas.identifier_akvo = AKVO_Tree_monitoring_areas.identifier_akvo
WHERE (AKVO_Tree_monitoring_areas.test = '' OR AKVO_Tree_monitoring_areas.test = 'This is real, valid data')
GROUP BY
akvo_tree_monitoring_counts.identifier_akvo,
...
--UNION OF COUNTS and PCQ's into 1 table
monitoring_instances_grouped AS
(SELECT * FROM pcq_instances_grouped
UNION
SELECT * FROM count_instances_grouped),
--ORDER BY akvo_tree_monitoring_areas.submission, akvo_tree_registration_areas.id_planting_site),
...
-- Create CTE to group the instances based on their strata label. This table makes sure that instances taken within
-- 180 days after another, are grouped into one instance (based on their strata label)
--monitoring_instances_merged AS (
...
-- Create a new selection table from monitoring results (both counts and PCQ), joining the their respective strata label
-- to each seperate instance
SELECT akvo_tree_registration_areas.identifier_akvo,
MAX(table_label_strata.submission) AS "Latest submission date",
...
WHEN AKVO_Tree_registration_areas.polygon ISNULL
THEN akvo_tree_monitoring_areas.avg_tree_height
END AS "Average tree height (m)",

ROUND((1/NULLIF(POWER(((AVG(Q1_dist) + AVG(Q2_dist) + AVG(Q3_dist) + AVG(Q4_dist))/4),2),0)*10000),0) AS "Tree density (trees/ha)",

CASE
WHEN AKVO_Tree_registration_areas.polygon NOTNULL
THEN CAST(ROUND((((1/NULLIF(POWER(((AVG(Q1_dist) + AVG(Q2_dist) + AVG(Q3_dist) + AVG(Q4_dist))/4),2),0))*10000)*AKVO_Tree_registration_areas.calc_area),0) AS INT)
WHEN AKVO_Tree_registration_areas.identifier_akvo = akvo_tree_monitoring_pcq.identifier_akvo
THEN CAST(ROUND((((1/NULLIF(POWER(((AVG(Q1_dist) + AVG(Q2_dist) + AVG(Q3_dist) + AVG(Q4_dist))/4),2),0))*10000)*AKVO_Tree_registration_areas.calc_area),0) AS INT)
WHEN AKVO_Tree_registration_areas.polygon ISNULL
THEN akvo_tree_monitoring_areas.number_living_trees
END AS "Total nr trees on site",

-- CALCULATE HERE THE PERCENTAGE INCREASE/DECLINE for PCQ sites
ROUND((ROUND((1/NULLIF(POWER(((AVG(Q1_dist) + AVG(Q2_dist) + AVG(Q3_dist) + AVG(Q4_dist))/4),2),0)*10000),0)*AKVO_Tree_registration_areas.calc_area/NULLIF(akvo_tree_registration_areas.tree_number,0)*100),0)
AS "Percentage of survived trees"
...
INNER JOIN
table_label_strata
ON table_label_strata.instance = akvo_tree_monitoring_pcq.instance
GROUP BY
...
-- Add the results from registrations to the upper table so that the initial registered tree numbers are integrated
-- including a '0' value for strata '0' (initial tree number). Only for polygons
SELECT
akvo_tree_registration_areas.identifier_akvo,
akvo_tree_registration_areas.submission AS "Latest submission date",
'0' AS monitoring_strata,
akvo_tree_registration_areas.id_planting_site,
akvo_tree_registration_areas.contract_number,
akvo_tree_registration_areas.organisation,
NULL as number_of_samples,
akvo_tree_registration_areas.calc_area as "Area (ha)",
...
akvo_tree_registration_areas.tree_number AS "Total nr trees on site",
...
akvo_tree_registration_areas.organisation,
NULL as number_of_samples,
akvo_tree_registration_areas.estimated_area as "Area (ha)",
...
ROUND(COUNT(*)/NULLIF(AKVO_Tree_registration_areas.estimated_area,0),2) AS "Photo density (photos/ha)",
...
CASE
WHEN MAX(CALC_TAB_monitoring_calculations_per_site_by_partner."Latest submission date") NOTNULL
