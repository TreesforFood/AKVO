...
CREATE TABLE IF NOT EXISTS ODK_unregistered_farmers_tree_registration_main (submissionid_odk TEXT, ecosia_site_id_dist TEXT, submitter TEXT, device_id TEXT, odk_form_version TEXT, submission_date DATE, field_date DATE, updated_at TIMESTAMPTZ, name_owner TEXT, id_planting_site TEXT, planting_date TEXT, test TEXT, planting_system_used TEXT, comment_planting_site TEXT, registration_multiple_locations TEXT, polygon geography(POLYGON, 4326), line geography(LINESTRING, 4326), point geography(POINT, 4326));
...
for json_in in json_unregistered_farmers_main:
    ...
    if json_extract(json_in, 'reporting_type')[0] == 'tree_hand_out':
        count = count+1
        print(json_in)
        # key_generated = json_extract(json_in, 'KEY')[0]
        # print(key_generated)
...
for json_in_tree_registration in json_unregistered_farmers_main:
    ...
    if json_extract(json_in_tree_registration, 'reporting_type')[0] == 'tree_registration':
        ...
        updated_at = json_extract(json_in_tree_registration, 'updatedAt')[0]
        ...
        cur.execute('''INSERT INTO ODK_unregistered_farmers_tree_registration_main (submissionid_odk, ecosia_site_id_dist, submitter, device_id, odk_form_version, submission_date, field_date, updated_at, name_owner, id_planting_site, planting_date, test, planting_system_used, comment_planting_site, registration_multiple_locations, polygon, line, point) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)''',
        (submissionid_odk, ecosia_site_id_dist, submitter, device_id, odk_form_version, submission_date, field_date, updated_at, name_owner, id_planting_site, planting_date, test, landscape_element_type, comment_planting_site, registration_multiple_locations, polygon_planting_site, line_planting_site, gps_center_planting_site))
