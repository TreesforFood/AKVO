...
-- For some instances there was no link between identifier_akvo and ecosia_site_id_dist. Probably an error in the (older) form. In order to still get the link, we created a link with farmer name or planting site id.
OR a.name_site_id_tree_planting = b.id_planting_site
OR LOWER(a.name_tree_receiver) = LOWER(b.name_owner)
JOIN CTE_odk_tree_count_unregistered_farmers c
ON b.submissionid_odk = c.submissionid_odk),
...
# The UPDATED table is maintained (not deleted) and only updated with NEW downloads (from the INTERATED table)
...
# UPDATE the UPDATE table with EDITS from the EDITS table (see next script below)
create_a1_updates = '''
...
WHERE NOT EXISTS (SELECT identifier_akvo FROM akvo_tree_registration_areas_edits
...
SET
polygon = akvo_tree_registration_areas_updated.polygon,
updated_at = akvo_tree_registration_areas_integrated.updated_at
FROM akvo_tree_registration_areas_integrated
...
#cur.execute(create_a1_integrate_new_data)
#cur.execute(create_a1_remote_sensing)
